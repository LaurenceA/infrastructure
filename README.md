...
These repos are set up by default to run on a node with general GPUs, but it is helpful to be able to train GPT2 on a single GPU for quick experiments. If you do in fact want to run with several GPUs and/or from checkpoint 8, notes [here](./ckpt-8-modded-nanogpt) may be useful.
